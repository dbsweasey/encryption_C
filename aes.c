/**
  Contains the AES algorithms for encryption and decryption.
  These algorithms are broken up into several small functions
  representing each step of the process.

  @file aes.c
  @file David Sweasey
*/

#include "aes.h"

/**
   Return the sBox substitution value for a given byte value.
   @param v byte input value.
   @return substitution for the given byte.
*/
static byte substBox( byte v )
{
  // Forward-direction replacement map for the sBox.
  static const byte rule[] =
    {  };

  // Return the replaceent for v based on the map.
  return rule[ v ];
}

/**
   Return the inverse sBox substitution value for a given byte value.
   @param v byte input value.
   @return inverse substitution for the given byte.
*/
static byte invSubstBox( byte v )
{
  // Inverse-direction replacement map for the sBox.
  static const byte irule[] =
  {  };
  
  // Return the (inverse) replaceent for v based on the map.
  return irule[ v ];
}

void gFunction( byte dest[ WORD_SIZE ], byte const src[ WORD_SIZE ], int r )
{
  // Constant values used in each round of the g function.
  static const byte roundConstant[ ROUNDS + 1 ] = {
    0x00, // First element not used.
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
  };
  byte result;
  dest[ WORD_SIZE - 1 ] = substBox( src[0] );
  for ( int i = 0; i < WORD_SIZE; i++ ) {
    result = substBox( src[i] );
    if ( i == 0 ) {
      dest[ WORD_SIZE - 1 ] = result;
      continue;
    } else if ( i == 1 ) {
      result = fieldAdd( result, roundConstant[r] );
    }
    dest[ i - 1 ] = result;
  }
}  

void generateSubkeys( byte subkey[ ROUNDS + 1 ][ BLOCK_SIZE ], byte const key[ BLOCK_SIZE ] )
{
  for ( int i = 0; i < BLOCK_SIZE; i++ ) {
    subkey[0][i] = key[i];
  }
  byte *word1 = (byte *)calloc( WORD_SIZE, sizeof(byte) );
  byte *word2 = (byte *)calloc( WORD_SIZE, sizeof(byte) );
  byte *word3 = (byte *)calloc( WORD_SIZE, sizeof(byte) );
  byte *word4 = (byte *)calloc( WORD_SIZE, sizeof(byte) );
  for ( int i = 1; i < ROUNDS + 1; i++ ) {
    for ( int j = WORD4; j < WORD4 + WORD_SIZE; j++ ) {
      word4[ j - WORD4 ] = subkey[i - 1][ j ];
      gFunction( word1, word4, i );
    }
    for ( int j = 0; j < WORD_SIZE; j++ ) {
      word1[ j ] = fieldAdd( word1[ j ], subkey[ i - 1 ][ j ] );
      subkey[ i ][ j ] = word1[ j ];
    }
    for ( int j = WORD2; j < WORD2 + WORD_SIZE; j++ ) {
      word2[ j - WORD2 ] = subkey[ i - 1 ][ j ];
    }
    for ( int j = 0; j < WORD_SIZE; j++ ) {
      word2[ j ] = fieldAdd( word2[ j ], word1[ j ] );
      subkey[ i ][ j + WORD2 ] = word2[ j ];
    }
    for ( int j = WORD3; j < WORD3 + WORD_SIZE; j++ ) {
      word3[ j - WORD3 ] = subkey[ i - 1 ][ j ];
    }
    for ( int j = 0; j < WORD_SIZE; j++ ) {
      word3[ j ] = fieldAdd( word3[ j ], word2[ j ] );
      subkey[ i ][ j + WORD3 ] = word3[ j ];
    }
    for ( int j = 0; j < WORD_SIZE; j++ ) {
      word4[ j ] = fieldAdd( word4[ j ], word3[ j ] );
      subkey[ i ][ j + WORD4 ] = word4[ j ];
    }
  }
  free( word1 );
  free( word2 ); 
  free( word3 );
  free( word4 );
}

void mixColumns( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  // Matrix by which each column of square is multiplied.
  static const byte mixMatrix[ BLOCK_ROWS ][ BLOCK_COLS ] = {
    { 0x02, 0x03, 0x01, 0x01 },
    { 0x01, 0x02, 0x03, 0x01 },
    { 0x01, 0x01, 0x02, 0x03 },
    { 0x03, 0x01, 0x01, 0x02 }
  };

  for ( int i = 0; i < BLOCK_COLS; i++ ) {
    byte column[ BLOCK_COLS ];
    for ( int j = 0; j < BLOCK_ROWS; j++ ) {
      byte result = 0;
      for ( int k = 0; k < BLOCK_COLS; k++ ) {
        result = fieldAdd( result, fieldMul( mixMatrix[j][k], square[k][i] ) );
      }
      column[j] = result;
    }
    for ( int j = 0; j < BLOCK_ROWS; j++ ) {
      square[j][i] = column[j];
    }
  }
}

void unMixColumns( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  // Matrix by which each column of square is multiplied.
  static const byte invMixMatrix[ BLOCK_ROWS ][ BLOCK_COLS ] = {
    { 0x0E, 0x0B, 0x0D, 0x09 },
    { 0x09, 0x0E, 0x0B, 0x0D },
    { 0x0D, 0x09, 0x0E, 0x0B },
    { 0x0B, 0x0D, 0x09, 0x0E }
  };

  for ( int i = 0; i < BLOCK_COLS; i++ ) {
    byte column[ BLOCK_COLS ];
    for ( int j = 0; j < BLOCK_ROWS; j++ ) {
      byte result = 0;
      for ( int k = 0; k < BLOCK_COLS; k++ ) {
        result = fieldAdd( result, fieldMul( invMixMatrix[j][k], square[k][i] ) );
      }
      column[j] = result;
    }
    for ( int j = 0; j < BLOCK_ROWS; j++ ) {
      square[j][i] = column[j];
    }
  }
  
}

void addSubkey( byte data[ BLOCK_SIZE ], byte const key[ BLOCK_SIZE ] )
{
  for ( int i = 0; i < BLOCK_SIZE; i++ ) {
    data[i] = fieldAdd( data[i], key[i] );
  }
}

void blockToSquare( byte square[ BLOCK_ROWS ][ BLOCK_COLS ], byte const data[ BLOCK_SIZE ])
{
  for ( int i = 0; i < BLOCK_COLS; i++ ) {
    for ( int j = 0; j < BLOCK_ROWS; j++ ) {
      square[j][i] = data[ j + i * BLOCK_COLS ];
    }
  }
}

void squareToBlock( byte data[ BLOCK_SIZE ], byte const square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  for ( int i = 0; i < BLOCK_SIZE; i++ ) {
    data[i] = square[ i % BLOCK_ROWS ][ i / BLOCK_COLS ];
  }
}

void shiftRows( byte square[ BLOCK_ROWS ][ BLOCK_COLS ])
{
  for ( int i = 1; i < BLOCK_ROWS; i++ ) {
    // Peform the left-shift i times where i is the column number
    for ( int j = 0; j < i; j++ ) {
      // Left-shift the row
      byte temp = square[i][0];
      for ( int k = 0; k < BLOCK_COLS; k++ ) {
        square[i][k] = square[i][ ( k + 1 ) % BLOCK_COLS ];
      }
      square[i][ BLOCK_COLS - 1 ] = temp;
    }
  }
}

void unShiftRows( byte square[ BLOCK_ROWS ][ BLOCK_COLS ])
{
  for ( int i = 1; i < BLOCK_ROWS; i++ ) {
    // Peform the right-shift i times where i is the column number
    for ( int j = 0; j < i; j++ ) {
      // Right-shift the row
      byte temp = square[i][0];
      for ( int k = BLOCK_COLS - 1; k >= 0; k-- ) {
        square[i][( k + 1 ) % BLOCK_COLS ] = square[i][k];
      }
      square[i][1] = temp;
    }
  }
}

void encryptBlock( byte data[ BLOCK_SIZE ], byte key[ BLOCK_SIZE ] )
{
  byte subkeys[ ROUNDS + 1 ][ BLOCK_SIZE ];
  byte square[ BLOCK_ROWS ][ BLOCK_COLS ];

  // Generate 11 subkeys
  generateSubkeys( subkeys, key );

  // First, add the first subkey 
  addSubkey( data, subkeys[0] );

  for ( int i = 1; i <= ROUNDS; i++ ) {
    // Step 1: Run each byte through the substitution function 
    for ( int j = 0; j < BLOCK_SIZE; j++ ) {
      data[j] = substBox( data[j] );
    }

    // Step 2: Block to square and perform shiftRows and mixColumns then return to block
    blockToSquare( square, data );
    shiftRows( square );
    // mixColumns is not perform on the last round
    if ( i != ROUNDS ) {
      mixColumns( square );
    }
    squareToBlock( data, square );

    // Step 3: Add the subkey
    addSubkey( data, subkeys[i] );
  }
}

void decryptBlock( byte data[ BLOCK_SIZE ], byte key[ BLOCK_SIZE ] ) 
{
  byte subkeys[ ROUNDS + 1 ][ BLOCK_SIZE ];
  byte square[ BLOCK_ROWS ][ BLOCK_COLS ];

  // Generate 11 subkeys
  generateSubkeys( subkeys, key );

  for ( int i = ROUNDS; i > 0; i-- ) {
    // Step 1: Add the subkey
    addSubkey( data, subkeys[i] );

    // Step 2: Block to square and perform mixColumns and shiftRows then return to block
    blockToSquare( square, data );
    // mixColumns is not performed on the first round
    if ( i != ROUNDS ) {
      unMixColumns( square );
    }
    unShiftRows( square );
    squareToBlock( data, square );

    // Step 3: Run each byte through the inverseSubstitution function
    for ( int j = 0; j < BLOCK_SIZE; j++ ) {
      data[j] = invSubstBox( data[j] );
    }
  }
  // Finally, add the first subkey
  addSubkey( data, subkeys[0] );
}
